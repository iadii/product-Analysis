import { Product, IProduct, Category } from '../models/index.js';
import { Outfit, RecommendationFilters } from '../types/index.js';
...
async function getCandidates(
    ...
    return Product.find(query).limit(limit).lean() as unknown as Promise<IProduct[]>;
}

async function getVariedAccessories(filters: RecommendationFilters, count: number = 4): Promise<IProduct[]> {
    const query: Record<string, unknown> = {
        category: 'accessory',
        inStock: true,
    };
    ...
    const allAccessories = await Product.find(query).limit(50).lean() as unknown as IProduct[];
