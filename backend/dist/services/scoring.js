...
function calculateStyleScore(styles1, styles2) {
    const common = styles1.filter(s => styles2.includes(s)).length;
    if (common > 0)
        return Math.min(0.85 + common * 0.05, 1.0);
    return 0.60;
}
function calculateOccasionScore(occasions1, occasions2) {
    const common = occasions1.filter(o => occasions2.includes(o)).length;
    return common > 0 ? 0.90 : 0.65;
}
function calculateSeasonScore(seasons1, seasons2) {
    if (seasons1.includes('all') || seasons2.includes('all'))
        return 0.95;
    const common = seasons1.filter(s => seasons2.includes(s)).length;
    return common > 0 ? 0.90 : 0.50;
}
function calculateBudgetScore(totalPrice, maxBudget) {
    if (!maxBudget)
        return 0.85;
    if (totalPrice <= maxBudget)
        return 1.0;
    if (totalPrice <= maxBudget * 1.3)
        return 0.7;
    return 0.4;
}
export function calculateOutfitScore(products, maxBudget) {
    ...
    const score = breakdown.colorHarmony * WEIGHTS.colorHarmony +
...
export function generateReasoning(products, breakdown) {
